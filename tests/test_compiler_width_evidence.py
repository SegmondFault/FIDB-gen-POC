...
class CompilerWidthEvidenceTests(unittest.TestCase):
    def test_snapshot_preserves_all_compiler_and_route_identities(self):
        ...
        path = (
            root
            / "toolchains/evidence/c-compiler-width-v1-reference-host-2026-09-02.toml"
        )
        ...
        self.assertEqual(
            sum(row["route_count"] for row in document["compiler"]), 29
        )
        self.assertEqual(
            sum(len(row["route_ids"]) for row in document["compiler"]), 29
        )
        self.assertEqual(

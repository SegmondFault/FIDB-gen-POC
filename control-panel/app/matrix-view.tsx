...
function PrioritySchedulePanel({ schedule }: { schedule: PrioritySchedule }) {
  ...
  return <section className="panel priority-schedule-panel">
    ...
    <div className="priority-schedule-facts"><span><b>{summary.cohorts}</b> cohorts · 10 + 10 + {summary.final_cohort_size}</span><span><b>{summary.source_families_cached}/{summary.source_families}</b> source families verified</span><span><b>{summary.recipe_families_prepared}/{summary.source_families}</b> preparation contracts ready</span><span><b>{summary.native_recipe_ready}</b> native · <b>{summary.runtime_provider_ready}</b> runtime providers</span><span><b>{summary.executable_provider_ready}/{summary.subjects}</b> executable providers ready</span><span><b>{summary.maximum_subject_executions.toLocaleString()}</b> maximum subject-width cells</span></div>
    <div className="priority-schedule-cohorts">
      {schedule.cohorts.map(cohort => <details key={cohort.id}>
        ...
        <section><header><span>Priority / subject</span><span>C80 relationship</span><span>Source</span><span>Execution provider</span><span>Qualification / plan</span><span>Current gate</span></header>{cohort.subjects.map(subject => <article key={subject.id}><div><b>#{subject.order} {subject.id}</b><small>{subject.source_family}{subject.aliases.length ? ` · ${subject.aliases.join(', ')}` : ''}</small></div><span className={subject.research_overlap ? 'pass' : 'new'}>{subject.research_overlap ? `C80 #${subject.research_rank}${subject.research_key !== subject.id ? ` · ${subject.research_key}` : ''}` : 'PRIORITY ADDITION'}</span><span className={subject.priority_source_cached ? 'pass' : subject.priority_source_pinned ? 'staged' : 'pending'}>{subject.priority_source_cached ? `VERIFIED · ${subject.priority_source_version}` : subject.priority_source_pinned ? `PINNED · ${subject.priority_source_version}` : 'PIN REQUIRED'}</span><span className={subject.recipe_state === 'reviewed-native' || (subject.recipe_state === 'reviewed-runtime' && subject.runtime_provider_blocked_cells === 0) ? 'pass' : subject.recipe_preparation_state ? 'staged' : 'pending'}>{subject.recipe_state.replaceAll('-', ' ').toUpperCase()}<br /><small>{subject.runtime_provider_kind ? `${subject.runtime_provider_ready_cells}/${subject.runtime_provider_cells} exact provider cells${subject.runtime_plan_path ? ` · ${subject.runtime_plan_path}` : ''}` : subject.width_batch_ids.length ? subject.width_batch_ids.join(' · ') : subject.planned_adapter ?? 'no adapter plan'}{subject.recipe_preparation_blockers.length ? ` · ${subject.recipe_preparation_blockers.length} blockers` : ''}</small></span><span className={subject.runtime_provider_kind ? (subject.runtime_provider_blocked_cells || !subject.runtime_plan_bound ? 'pending' : 'pass') : subject.qualification_satisfied ? 'pass' : 'pending'}>{subject.runtime_provider_kind ? (subject.runtime_provider_blocked_cells ? `${subject.runtime_provider_blocked_cells} BLOCKED` : subject.runtime_plan_bound ? 'PLAN DISARMED' : 'PLAN REQUIRED') : subject.qualification_satisfied ? 'SEALED' : subject.width_batch_bound ? 'DEFINED · DISARMED' : 'REQUIRED'}</span><strong>{stageLabel(subject.stage)}</strong></article>)}</section>
      </details>)}
    </div>

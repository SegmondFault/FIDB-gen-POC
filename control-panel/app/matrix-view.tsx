...
function PrioritySchedulePanel({ schedule }: { schedule: PrioritySchedule }) {
  ...
  return <section className="panel priority-schedule-panel">
    ...
    <div className="priority-schedule-facts"><span><b>{summary.cohorts}</b> cohorts · 10 + 10 + {summary.final_cohort_size}</span><span><b>{summary.source_families_cached}/{summary.source_families}</b> source families verified</span><span><b>{summary.recipe_families_prepared}/{summary.source_families}</b> recipe designs prepared</span><span><b>{summary.native_recipe_ready}</b> executable recipes · <b>{summary.limited_recipe_only}</b> limited</span><span><b>{summary.maximum_subject_executions.toLocaleString()}</b> maximum subject-width cells</span></div>

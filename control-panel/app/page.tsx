...
function MachineValidationView({ factory, navigateTo }: { factory: FactoryApiState; navigateTo: (view: string) => void }) {
  ...
  return <div className="view-stack machine-validation-view">
    ...
    <section className="panel validation-results-panel validation-outcome-panel"><header><h3>Attribution outcome · {validation.id}</h3><div><button className="text-button" onClick={() => navigateTo('Hash discrimination')}>Hash discrimination →</button><span className={`validation-state ${validation.results.state === 'measured-complete' ? 'ready' : 'waiting'}`}>{reportState}</span></div></header>
      <div className="validation-rate-grid"><article><span>PRECISION</span><strong>{validationRate(tp, tp === null || fp === null ? null : tp + fp)}</strong><small>accepted owners that were correct</small></article><article><span>RECALL</span><strong>{validationRate(tp, tp === null || fn === null ? null : tp + fn)}</strong><small>expected owner evidence recovered</small></article><article><span>FALSE-POSITIVE RATE</span><strong>{validationRate(fp, fp === null || tn === null ? null : fp + tn)}</strong><small>wrong-owner decisions accepted</small></article><article><span>MEASURED DECISIONS</span><strong>{measuredDecisions === null ? '—' : measuredDecisions.toLocaleString()}</strong><small>{reportPending ? `${runOutstanding} work units unresolved` : matrix.unit.replaceAll('-', ' ')}</small></article><article><span>COLLISIONS / MISSES</span><strong>{validation.results.state === 'measured-complete' ? `${validation.results.failure_summary.collisions} / ${validation.results.failure_summary.misses}` : '—'}</strong><small>specific failures retained below</small></article><article><span>NOISY HASH CANDIDATES</span><strong>{noisyCandidates === null ? '—' : noisyCandidates.toLocaleString()}</strong><small>cross-run trust evidence</small></article></div>
...
function RetentionView({ factory }: { factory: FactoryApiState }) {
  ...
  return <div className="view-stack retention-view">
    ...
    <section className="retention-metrics">
      <article className="panel"><span>SELECTED</span><strong>{plan ? formatBytes(plan.summary.recoverable_apparent_bytes) : '—'}</strong><small>{plan ? `${formatBytes(plan.summary.validation_recoverable_apparent_bytes ?? 0)} validation scratch · ${plan.summary.actions.toLocaleString()} actions` : 'build a dry-run'}</small></article>

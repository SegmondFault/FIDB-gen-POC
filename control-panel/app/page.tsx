...
function MachineValidationView({ factory }: { factory: FactoryApiState }) {
  ...
  return <div className="view-stack machine-validation-view">
    ...
    <section className="panel validation-status-panel">
      ...
      <div className="validation-headline-metrics"><article><span>MODE</span><strong>{run.mode?.toUpperCase() ?? '—'}</strong><small>{run.run_id ?? 'no run admitted'}</small></article><article><span>WORK UNITS</span><strong>{run.complete_work_units} / {runExpected || '—'}</strong><small>{run.failed_work_units} failed</small></article><article><span>CANARY GATE</span><strong>{validation.canary_gate.ready ? 'PASSED' : 'REQUIRED'}</strong><small>{validation.canary_gate.run_id ?? 'current runtime not qualified'}</small></article><article><span>WORKERS</span><strong>{runActive ? run.worker_pids?.length ?? 0 : 0}</strong><small>{run.state === 'paused' ? 'checkpointed and quiet' : 'isolated Ghidra processes'}</small></article><article><span>STARTED</span><strong>{run.started_at ? new Date(run.started_at).toLocaleTimeString() : '—'}</strong><small>{run.paused_at ? `paused ${new Date(run.paused_at).toLocaleTimeString()}` : run.finished_at ? `finished ${new Date(run.finished_at).toLocaleTimeString()}` : 'local browser time'}</small></article><article><span>REPORT</span><strong>{run.report_path && run.state === 'complete' ? 'READY' : '—'}</strong><small>{run.state === 'paused' ? 'partial evidence retained' : run.report_path ?? run.error ?? 'awaiting measured output'}</small></article></div>

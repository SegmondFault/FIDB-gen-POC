...
function HashFormulaPanel({ hdi }: { hdi: HashDiscriminationStatus }) {
  ...
  return <section className="panel hdi-formula-panel">
    <header><div><p className="panel-kicker">HOW THE VALUES ARE CALCULATED</p><h3>Transparent weighted components—not an opaque model</h3><small>Every component is normalised to {hdi.model.component_value_minimum}–{hdi.model.component_value_maximum}. These provisional C10 weights are inactive until the required evidence exists.</small></div><span>{hdi.reproducibility.algorithm_id}</span></header>
...
function HashDiscriminationView({ factory }: { factory: FactoryApiState }) {
  ...
  const [message, setMessage] = useState('');
  if (!hdi || !ledger) return <div className="view-stack"><ViewIntro kicker="HASH DISCRIMINATION" title="Learn how strongly each FID hash distinguishes library provenance" copy="The Hash Discrimination authority is unavailable." /><section className="panel"><div className="empty-state"><span>◇</span><strong>No HDI authority loaded</strong><p>Reconnect the local API or inspect validation/hash-discrimination.toml.</p></div></section></div>;
...
function ToolchainsView({ factory, selectedLanguageId, setSelectedLanguageId }: { factory: FactoryApiState; selectedLanguageId: string; setSelectedLanguageId: React.Dispatch<React.SetStateAction<string>> }) {
  ...
  return <div className="view-stack">
    ...
    {compilerWidthPlan && <section className="panel compiler-generation-panel">
      <div className="panel-header"><div><p className="panel-kicker">EXACT COMPILER WIDTH · {compilerWidthPlan.profile.id}</p><h3>Target and compiler generation are independent axes</h3></div><span className="plan-state">{compilerWidthPlan.summary.qualified_routes} / {compilerWidthPlan.summary.routes} ROUTES QUALIFIED</span></div>

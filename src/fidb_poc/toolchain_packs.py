...
CATALOG_SCHEMA = "fidb-toolchain-pack-catalog/v1"
PROFILE_PLAN_SCHEMA = "fidb-toolchain-profile-plan/v1"
...
def _load_routes(path: Path) -> list[dict[str, object]]:
    ...
    for row in rows:
        route_id = str(row["id"])
        for field in _ROUTE_FIELDS - {"pack_ids", "external_requirements"}:
        ...
        if row["provisioning"] not in {"downloadable-pack", "external-worker"}:
            raise ValueError(f"route {route_id} has invalid provisioning")
        if row["provisioning"] == "downloadable-pack" and not row["pack_ids"]:
            raise ValueError(f"downloadable route {route_id} requires a pack")
        if row["provisioning"] == "external-worker" and (
            row["pack_ids"] or not row["external_requirements"]
        ):
            raise ValueError(
...
def load_toolchain_pack_catalog(project_root: str | Path) -> dict[str, object]:
    ...
    for profile in profiles:
        ...
        if not set(profile["route_ids"]).issubset(requirement_ids):
            raise ValueError(
                f"profile {profile['id']} routes are not a subset of its width authority"
            )
...
def resolve_toolchain_profile(
    ...
    *,
    ...
    for route in route_rows:
        if route["provisioning"] != "downloadable-pack":
            continue
        ...
        route["state"] = (
            ...
            if "broken" in states
                ...
                "verified-cached"
                if all(state == "verified-cached" for state in states)
    ...
    elif any(row["state"] == "external-required" for row in route_rows):
    ...
    total_installed = sum(int(row["installed_bytes_estimate"]) for row in pack_rows)
    cached_download = sum(
    ...
    return {
        ...
        "summary": {
            ...
            "downloadable_routes": sum(
                row["provisioning"] == "downloadable-pack" for row in route_rows
            ),
            "external_routes": sum(

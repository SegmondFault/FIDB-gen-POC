...
def _score(strings: tuple[str, ...]) -> int:
    ...
    language = sum(
        1 for word in re.findall(r"[a-z]{4,}", joined) if word in natural_words
    )
...
def investigate(path: str | Path) -> Investigation:
    ...
    if "busybox" in decoded_text and "/proc/" in decoded_text:
        insights.append(
            "Decoded process-management and BusyBox strings support a Mirai-family payload."
        )
    if "watchdog" in decoded_text:
        insights.append(
            "Decoded watchdog device paths indicate embedded Linux persistence handling."
        )
    if facts.statically_linked:
        insights.append(
            "Static linkage makes the C runtime the highest-value initial FID family."
        )
    ...
    if facts.statically_linked:
        hypotheses.extend(
            (
                Hypothesis(
                    "c-runtime",
                    "high" if libc_evidence else "medium",
                    "build",
    ...
    for package, markers in PACKAGE_MARKERS.items():
        ...
        hypotheses.append(
            Hypothesis(
                ...
                (
                    f"Observed package markers: {', '.join(found)}."
                    if found
                    else "No package-specific marker was observed; absence is not proof of exclusion."
                ),

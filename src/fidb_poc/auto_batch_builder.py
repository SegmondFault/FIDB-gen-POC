...
def _render_queue(document: dict[str, object], source_queue: dict[str, object]) -> str:
    ...
    batch_order = [str(chunk["id"]) for chunk in document["chunks"]]  # type: ignore[index]
    lines = [
        ...
        f"name = {_toml_value(str(document['id']) + '-queue')}",
        "",
        "[queue]",
        "armed = false",
